...
def parse_args():
    ...
    parser.add_argument("--early_weights", type=list, default=[0.5, 0.5])
    parser.add_argument("--latter_weights", type=list, default=[0.0, 1.0])
...
def main():
    ...
    axs[2].set_title(f"ReNoise ({psnr_score:.2f} dB)")
    axs[2].imshow(np.array(recon_image))
    axs[3].set_title(f"ReNoise-Enhance Edit ({psnr_score_e:.2f} dB)")
