...
from inversions.unet_based.inv_dpm import SDInversionPipeline, SDXLInversionPipeline, image2latents, latents2image
...
def main():
    ...
    if args.model_type == "SDv1.5" or args.model_type == "SDXLv1.0":
        assert args.prediction_type == "epsilon"
    if args.model_type == "SDv2.1":
    ...
    inv_result = pipe.inverse(
        ...
        perform_decoder_inversion=False,
    ...
    inv_result = pipe.inverse(
        ...
        solver_order=args.solver_order,
        step_size=args.step_size,
        perform_decoder_inversion=True,
    ...
    print(f"[Inv-DPM] PSNR: {psnr_score:.2f}, SSIM: {ssim_score:.4f}, LPIPS: {lpips_score:.4f}")

    psnr_score_d = psnr(np.array(ori_image), np.array(recon_image_decoder_inv))
    ssim_score_d = ssim(np.array(ori_image), np.array(recon_image_decoder_inv), win_size=11, channel_axis=2)
    lpips_score_d = lpips_loss(pil2tensor(ori_image), pil2tensor(recon_image_decoder_inv)).item()
    print(f"[Inv-DPM w/ Decoder Inv] PSNR: {psnr_score_d:.2f}, SSIM: {ssim_score_d:.4f}, LPIPS: {lpips_score_d:.4f}")
    ...
    axs[2].set_title(f"Inv-DPM ({psnr_score:.2f} dB)")
    axs[2].imshow(np.array(recon_image))
    axs[3].set_title(f"Inv-DPM w/ Decoder Inv ({psnr_score_d:.2f} dB)")
    axs[3].imshow(np.array(recon_image_decoder_inv))
