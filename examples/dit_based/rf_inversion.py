...
from inversions.dit_based.rf_inversion import RFInversionFluxPipeline, ImageLoader, image2latent, latent2image
...
def parse_args():
    ...
    parser.add_argument("--stop_timestep", type=float, default=1.0)
...
def main():
    ...
    inverted_latents, image_latents, latent_image_ids = pipe.inverse(
        image=ori_image,
        source_prompt="",
    ...
    print(f"[RF-Inversion] PSNR: {psnr_score:.2f}, SSIM: {ssim_score:.4f}, LPIPS: {lpips_score:.4f}")
    ...
    axs[2].set_title(f"RF-Inversion ({psnr_score:.2f} dB)")
