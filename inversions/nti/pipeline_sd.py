        ...
        **kwargs,
        ...
        with EmojiProgressBar(total=num_inference_steps * num_optim_steps, desc="Null-Text Inversion") as pbar:
            for i in range(num_inference_steps):
                ...
                optimizer = torch.optim.Adam([negative_prompt_embeds], lr=lr*(1.-i/100.))
                with torch.no_grad():
        ...
        return InversionPipelineOutput(zT=inversed_latents[-1], uncond_embeds=uncond_embeds, ori_image=image)
    
    @torch.no_grad()
    def __call__(
        self,
        uncond_embeds: List[torch.Tensor],
        prompt: Union[str, List[str]] = None,
        height: Optional[int] = None,
        width: Optional[int] = None,
        num_inference_steps: int = 50,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=torch.cat([uncond_embeds[i], prompt_embeds]),
                    timestep_cond=timestep_cond,
                    cross_attention_kwargs=self.cross_attention_kwargs,
                    added_cond_kwargs=added_cond_kwargs,
                    return_dict=False,
