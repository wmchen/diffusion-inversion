...
class RFSolverFluxPipeline(FluxPipeline):
    @torch.no_grad()
    def inverse(
        self,
        ...
        # 6. Denoising loop
        with EmojiProgressBar(total=num_inference_steps, desc="RF-Solver (inversion)") as pbar:
            for t in timesteps:
                if self.interrupt:
                    continue
                ...
                # calculate the latent at t+▲t
                sigma_cur = self.scheduler.sigmas[self.scheduler.step_index]
                sigma_next = self.scheduler.sigmas[self.scheduler.step_index+1]
                sigma_delta = 0.5 * (sigma_cur + sigma_next)
                sample = latents.to(torch.float32)
                latents_delta = sample + (sigma_delta - sigma_cur) * v
                latents_delta = latents_delta.to(v.dtype)

                # calculate the velocity at t+▲t
                t_delta = sigma_delta * self.scheduler.config.num_train_timesteps
                timestep_delta = t_delta.expand(latents.shape[0]).to(latents.dtype)
                v_delta = self.transformer(
                    hidden_states=latents_delta,
                    timestep=timestep_delta / 1000,
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                    encoder_hidden_states=prompt_embeds,
                    txt_ids=text_ids,
                ...
                # calculate v prime
                v_prime = (v_delta - v) / (0.5 * (sigma_next - sigma_cur))

                # calculate the latent
                sample = latents.to(torch.float32)
                latents = sample + (sigma_next - sigma_cur) * v + 0.5 * (sigma_next - sigma_cur) ** 2 * v_prime
                latents = latents.to(v.dtype)
    ...
    def __call__(
        ...
        # 6. Denoising loop
        with EmojiProgressBar(total=num_inference_steps, desc="RF-Solver (denoise)") as pbar:
            for t in timesteps:
                if self.interrupt:
                    continue
