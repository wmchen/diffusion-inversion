...
@INVERSION.register_module("NTI")
def null_text_inversion(image: Union[str, pil.Image],
    ...
    """Null-Text Inversion (NTI): https://arxiv.org/abs/2211.09794
    ...
    with EmojiProgressBar(total=num_inference_steps*num_optim_steps, desc="NTI") as pbar:
        for i in range(num_inference_steps):
            ...
            uncond_prompt_emb.requires_grad = True
            optimizer = optim.Adam([uncond_prompt_emb], lr=lr*(1.-i/100.))
            ...
            for _ in range(num_optim_steps):
                ...
                pbar.update(1)
            uncond_embeddings.append(uncond_prompt_emb.detach())
            with torch.no_grad():
    ...
    outputs.uncond_embeddings = uncond_embeddings
