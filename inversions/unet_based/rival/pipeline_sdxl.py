...
from .hook import AttentionQKVHook, AttentionSelfAttnKVInjector
...
class SDXLInversionPipeline(StableDiffusionXLPipeline):
    ...
    def inverse(
        ...
        **kwargs,
        ...
        # 10. Apply attention hook
        self_attentions = []
        attn_hook = AttentionQKVHook(self.unet)
        attn_hook.apply()

        self._num_timesteps = len(timesteps)
        with EmojiProgressBar(total=num_inference_steps, desc="RIVAL") as pbar:
            for i in range(num_inference_steps):
                ...
                # cache self attention maps
                self_attentions.append(attn_hook.self_attn)
        ...
        # 11. Remove attention hook
        attn_hook.remove()

        # Offload all models
        self.maybe_free_model_hooks()

        return InversionPipelineOutput(zT=latents/self.scheduler.init_noise_sigma, cache_attentions=self_attentions, ori_image=image)
    
    @torch.no_grad()
    def __call__(
        self,
        cache_attentions: List[tuple],
        t_align: int = 600,
        prompt: Union[str, List[str]] = None,
        prompt_2: Optional[Union[str, List[str]]] = None,
        height: Optional[int] = None,
        ...
        **kwargs,
        ...
        # 10. Apply attention injector
        attn_injector = AttentionSelfAttnKVInjector(self.unet, t_align)
        attn_injector.apply()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i in range(num_inference_steps):
                t = timesteps[i]
                attn_injector.self_attn = cache_attentions[num_inference_steps-i-1]
                attn_injector.cur_timestep = t.item()
        ...
        # 11. Remove attention injector
        attn_injector.remove()
