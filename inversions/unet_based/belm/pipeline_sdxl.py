...
class SDXLInversionPipeline(StableDiffusionXLPipeline):
    ...
    def inverse(
        ...
        **kwargs,
        ...
        xis = [latents]
        with EmojiProgressBar(total=num_inference_steps, desc="BELM") as pbar:
            for i in range(num_inference_steps):
                index = num_inference_steps - i - 1
                t = timesteps[index+1] if index < num_inference_steps - 1 else 1
                ...
                if i == 0:
                    ...
                    alpha_i_minus_2 = 1 if i == 1 else self.scheduler.alphas_cumprod[timesteps[index + 2]].to(torch.float32)
                    ...
                    latents = coef_x_i_minus_2 * xis[-2] + coef_x_i_minus_1 * xis[-1] + coef_eps * noise_pred
                    xis.pop(0)
                xis.append(latents)
        ...
        return InversionPipelineOutput(
            zT=xis[-1] / self.scheduler.init_noise_sigma, 
            zT_second=xis[-2] / self.scheduler.init_noise_sigma, 
            ori_image=image
        )
    ...
    def __call__(
        ...
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        latents: Optional[torch.Tensor] = None,
        latents_second: Optional[torch.Tensor] = None,
        prompt_embeds: Optional[torch.Tensor] = None,
        negative_prompt_embeds: Optional[torch.Tensor] = None,
        pooled_prompt_embeds: Optional[torch.Tensor] = None,
        negative_pooled_prompt_embeds: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        latents_second = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
            width,
