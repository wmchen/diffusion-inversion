        ...
        **kwargs,
        ...
        with EmojiProgressBar(total=num_inference_steps * num_optim_steps, desc="Prompt Tuning Inversion") as pbar:
            for i in range(num_inference_steps):
                ...
                optimizer = torch.optim.Adam([prompt_embeds], lr=lr*(1.-i/100.))
                with torch.no_grad():
        ...
        return InversionPipelineOutput(zT=inversed_latents[-1], cond_embeds=cond_embeds, ori_image=image)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=torch.cat([negative_prompt_embeds, cond_embeds[i]]),
                    timestep_cond=timestep_cond,
                    cross_attention_kwargs=self.cross_attention_kwargs,
                    added_cond_kwargs=added_cond_kwargs,
                    return_dict=False,
