...
class SDXLInversionPipeline(StableDiffusionXLPipeline):
    ...
    def inverse(
        ...
        **kwargs,
        ...
        with EmojiProgressBar(total=num_inference_steps, desc="BDIA") as pbar:
            for i in range(num_inference_steps):
                ...
                if i == 0:
                    ...
                    next_latents = self.scheduler.ddim_inverse_step(noise_pred, t, latents)  # z_{t+1}^hat

                    if num_inference_steps-i-2 >= 0:
                        t_next = timesteps[num_inference_steps-i-2]
                        ...
                        # perform guidance
                        if self.do_classifier_free_guidance:
                            noise_pred_uncond, noise_pred_text = noise_pred_next.chunk(2)
                            noise_pred_next = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)

                        if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                            # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                            noise_pred_next = rescale_noise_cfg(noise_pred_next, noise_pred_text, guidance_rescale=self.guidance_rescale)
                        
                        latents_recon = self.scheduler.step(noise_pred_next, t_next, next_latents, **extra_step_kwargs, return_dict=False)[0]
                        latents = latents / gamma - latents_recon / gamma + next_latents
                    else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i in range(num_inference_steps):
                ...
                if i == 0:
                    ...
                    prev_latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]  # z_{t-1}^hat
                    ...
                    next_latents_recon = self.scheduler.ddim_inverse_step(noise_pred_next, t_next, latents)  # z_{t+1}^hat

                    latents = gamma * next_latents.pop(0) - gamma * next_latents_recon + prev_latents
                t_next = t
                next_latents.append(latents.clone())
