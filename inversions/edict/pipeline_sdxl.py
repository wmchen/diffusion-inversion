...
class SDXLInversionPipeline(StableDiffusionXLPipeline):
    ...
    def inverse(
        ...
        **kwargs,
        ...
        return InversionPipelineOutput(latents_pair=latents_pair, ori_image=image)
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # calculate x_inter
                latent_model_input = torch.cat([y] * 2) if self.do_classifier_free_guidance else y
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                ...
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
                x_inter = self.scheduler.denoise_step(noise_pred, t, x)
                ...
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
                y_inter = self.scheduler.denoise_step(noise_pred, t, y)

                x_prev = p * x_inter + (1 - p) * y_inter
                y_prev = p * y_inter + (1 - p) * x_prev

                latents_pair = [y_prev.clone(), x_prev.clone()]

                # call the callback, if provided
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()

        images = [latents2image(self, latent) for latent in latents_pair]
